...
begin
	# Create DataDrivenProblem for each NFB case
	ngf_dd_prob = gf_dd_prob(df_NGF[rand(1:nrow(df_NGF), nrow(df_NGF)),:])
...
# ╠═╡ disabled = true
#=╠═╡
function DataDrivenDiffEq.get_parameter_map(x::DataDrivenDiffEq.Basis)
    map(DataDrivenDiffEq.parameters(x)) do p
		if eltype(p) == Any
			p = Vector{Float64}(p)
		end
...
ps = DataDrivenDiffEq.get_parameter_values(gf_basis)

# ╔═╡ 767f9e0c-5763-4c6c-a66d-d6b68f39a6ad
dd_test = DataDrivenDiffEq.remake_problem(ngf_dd_prob, p=ps)

# ╔═╡ 8460a7f6-caf0-4c7b-968b-450c67874efb
DataDrivenDiffEq.get_implicit_data(dd_test)

# ╔═╡ e15b5b95-afb0-4fc7-91f7-4e23e0ab3646
begin
	mutable struct ImplicitOpt{T <: DataDrivenSparse.AbstractSparseRegressionAlgorithm} <:
	               DataDrivenSparse.AbstractSparseRegressionAlgorithm
	    """Explicit Optimizer"""
	    optimizer::T
	
	    function ImplicitOpt(threshold = 1e-1, opt = STLSQ)
	        optimizer = opt(threshold)
			
	        return new{typeof(optimizer)}(optimizer)
	    end
	
	    function ImplicitOpt(opt::T) where {T <: DataDrivenSparse.AbstractSparseRegressionAlgorithm}
	        return new{T}(opt)
			
	    end
	end
	
	Base.summary(opt::ImplicitOpt) = "Implicit Optimizer using " * summary(opt.optimizer)
	
	get_threshold(opt::ImplicitOpt) = get_threshold(opt.optimizer)
	
	function (x::ImplicitOpt)(X, Y; options::DataDrivenSparse.DataDrivenCommonOptions = 
								DataDrivenSparse.DataDrivenCommonOptions(),
								necessary_idx = ones(Bool, size(X, 1)),
								kwargs...)
	    @unpack optimizer = x
	    @unpack verbose = options

		#println(necessary_idx)
	    n, _ = size(X)  # n = nb features
	
	    x_opt = zeros(eltype(X), 1, n) # All solutions
	    inds = [false for _ in 1:n]
		
	
	    solver = DataDrivenSparse.SparseLinearSolver(optimizer, options = options)
	    ...
	    best_id = 0
		best_aicc = DataDrivenSparse.aicc(first(results[1]))
	    foreach(enumerate(results)) do (i, res)
			#println(res)
	        inds .= true
	        inds[i] = false
	        if DataDrivenSparse.dof(first(res)) >= 2 && any(!iszero(DataDrivenSparse.coef(first(res))[:, necessary_idx[inds]]))
	            if best_id <= 0
	                best_id = i
	            elseif DataDrivenSparse.aicc(first(res)) < DataDrivenSparse.aicc(first(results[best_id]))
	                best_id = i
	            end
			else
				println(i)
				println(DataDrivenSparse.dof(first(res)) >= 2)
				println(DataDrivenSparse.dof(first(res)))
				println(any(!iszero(DataDrivenSparse.coef(first(res))[:, necessary_idx[inds]])))
	        end
	    end
	    # Build the coefficient matrix
		println("best_id $(best_id)")
	    inds .= true
	    inds[best_id] = false
	    ...
	    x_opt[1:1, inds] .= DataDrivenSparse.coef(best_cache)
...
function objective(trial, dd_prob, basis, implicit=false)
	...
	if implicit
		res_dd = DataDrivenDiffEq.solve(dd_prob, 
			basis, 
			ImplicitOpt(DataDrivenSparse.STLSQ()), 
			options = options)
...
# ╠═╡ show_logs = false
begin
	opt = ImplicitOpt(DataDrivenSparse.SR3(0.4, 0.001))
	for i in 1:1
		DataDrivenDiffEq.solve(ngf_dd_prob, gf_basis, opt, options = options)
	end
end
...
function sindy_bootstrap(df, basis, n_bstraps, create_dd_prob, implicit=false)
	...
	for i in 1:n_bstraps
		...
		#try
		if implicit
			best_res = DataDrivenDiffEq.solve(dd_prob, basis, ImplicitOpt(DataDrivenSparse.STLSQ()), options = options) 
...
#=╠═╡
begin
	# For each case, find the best hyperparameter
	hp_res_noFB = HyperTuning.optimize(trial -> objective(trial, dd_prob_noFB), scenario) 
	hp_res_a = HyperTuning.optimize(trial -> objective(trial, dd_prob_a), scenario) 
	hp_res_b = HyperTuning.optimize(trial -> objective(trial, dd_prob_b), scenario) 
	hp_res_ab = HyperTuning.optimize(trial -> objective(trial, dd_prob_ab), scenario) 
end
  ╠═╡ =#
...
#=╠═╡
begin
	# Retrieve the solution with the optimal hyperparameter
	res_dd_noFB = DataDrivenDiffEq.solve(dd_prob_noFB, basis, DataDrivenSparse.SR3(hp_res_noFB.best_trial.values[:λ], hp_res_noFB.best_trial.values[:ν], DataDrivenSparse.HardThreshold()), options = options)
	
	res_dd_a = DataDrivenDiffEq.solve(dd_prob_a, basis, DataDrivenSparse.SR3(hp_res_a.best_trial.values[:λ], hp_res_a.best_trial.values[:ν], DataDrivenSparse.HardThreshold()), options = options)
	
	res_dd_b = DataDrivenDiffEq.solve(dd_prob_b, basis, DataDrivenSparse.SR3(hp_res_b.best_trial.values[:λ], hp_res_b.best_trial.values[:ν], DataDrivenSparse.HardThreshold()), options = options)
	
	res_dd_ab = DataDrivenDiffEq.solve(dd_prob_ab, basis, DataDrivenSparse.SR3(hp_res_ab.best_trial.values[:λ], hp_res_ab.best_trial.values[:ν], DataDrivenSparse.HardThreshold()), options = options)
end
  ╠═╡ =#

# ╔═╡ ade5f931-5649-4717-8c15-d4f861dbdb91
#=╠═╡
res_dd_noFB.out[1].coefficients
  ╠═╡ =#

# ╔═╡ b0c2d01a-ba70-4090-a40d-6ade837cb4e8
res_dd_a.out

# ╔═╡ d159de1b-4e84-403a-8e67-776f64a76763
# ╠═╡ disabled = true
#=╠═╡
begin

	#=res_dd_noFB = DataDrivenDiffEq.solve(dd_prob_noFB, basis, DataDrivenSparse.ADMM(1e-2:1e-1:5e-1), options = options)
	res_dd_a = DataDrivenDiffEq.solve(dd_prob_a, basis, DataDrivenSparse.ADMM(1e-2:1e-1:5e-1), options = options)
	res_dd_b = DataDrivenDiffEq.solve(dd_prob_b, basis, DataDrivenSparse.ADMM(1e-2:1e-1:5e-1), options = options)
	res_dd_ab = DataDrivenDiffEq.solve(dd_prob_ab, basis, DataDrivenSparse.ADMM(1e-2:1e-1:5e-1),
				options = options)=#

	#res_dd_noFB = DataDrivenDiffEq.solve(dd_prob_noFB, basis, DataDrivenSparse.SR3(x, 1.0, DataDrivenSparse.HardThreshold()), options = options)
	
	res_dd_noFB = DataDrivenDiffEq.solve(dd_prob_noFB, basis, DataDrivenSparse.SR3(1e-3:5e-3:5e-1, 1, DataDrivenSparse.HardThreshold()), options = options)

	res_dd_b = DataDrivenDiffEq.solve(dd_prob_b, basis, DataDrivenSparse.SR3(1e-3:5e-3:5e-1, 1, DataDrivenSparse.HardThreshold()), options = options)

	res_dd_ab = DataDrivenDiffEq.solve(dd_prob_ab, basis, DataDrivenSparse.SR3(1e-3:5e-3:5e-1, 1, DataDrivenSparse.HardThreshold()), options = options)
	
	#=res_dd_noFB = DataDrivenDiffEq.solve(dd_prob_noFB, basis, DataDrivenSparse.STLSQ(1e-2:1e-1:5e-1), options = options)
	res_dd_a = DataDrivenDiffEq.solve(dd_prob_a, basis, DataDrivenSparse.STLSQ(1e-2:1e-1:5e-1), options = options)
	res_dd_b = DataDrivenDiffEq.solve(dd_prob_b, basis, DataDrivenSparse.STLSQ(1e-2:1e-1:5e-1), options = options)
	res_dd_ab = DataDrivenDiffEq.solve(dd_prob_ab, basis, DataDrivenSparse.STLSQ(1e-2:1e-1:5e-1),
	            options = options)=#
end
  ╠═╡ =#

# ╔═╡ 3600ce6b-02be-4a61-a438-5fd99da61f9e
md"""
#### Case noFB
"""

# ╔═╡ 495afb30-f9ba-4c28-ab92-2e931ac22494
#=╠═╡
res_dd_noFB.basis
  ╠═╡ =#

# ╔═╡ cda77c32-b59b-42e1-be68-b5da3784f7f2
#=╠═╡
plot(res_dd_noFB)
  ╠═╡ =#

# ╔═╡ 191b056b-e11f-4c14-acff-3092a0c9078c
#=╠═╡
begin
	eqs_noFB = DataDrivenSparse.get_basis(res_dd_noFB)
	println(eqs_noFB)
	param_noFB = DataDrivenSparse.get_parameter_map(eqs_noFB)
	println(param_noFB)
end
  ╠═╡ =#

# ╔═╡ e482fe57-8a21-4886-a791-027b055f1eee
md"""
#### Case a
"""

# ╔═╡ fb8e00ec-8535-4ba5-8ea2-ef60b579e057
plot(res_dd_a)

# ╔═╡ 6886290f-e948-4398-884d-3058fb7d3f7b
begin
	eqs_a = DataDrivenSparse.get_basis(res_dd_a)
	println(eqs_a)
	param_a = DataDrivenSparse.get_parameter_map(eqs_a)
	println(param_a)
end

# ╔═╡ 42ef886f-7694-4275-99d3-cbc436496692
md"""
#### Case b
"""

# ╔═╡ 9e30e44a-a5dc-4428-a4e4-aebb15482973
#=╠═╡
plot(res_dd_b)
  ╠═╡ =#

# ╔═╡ e98e3047-b504-4853-b146-3aafe5f67bb7
#=╠═╡
begin
	eqs_b = DataDrivenSparse.get_basis(res_dd_b)
	println(eqs_b)
	param_b = DataDrivenSparse.get_parameter_map(eqs_b)
	println(param_b)
end
  ╠═╡ =#
...
git-tree-sha1 = "24aef32b3eab9f7db146e00220be50dd60ded125"

...
begin
	using ModelingToolkit, SciMLSensitivity, Optimization, OptimizationOptimisers, OptimizationOptimJL, LineSearches
	
	# External libraries
	using Lux, Zygote, JLD2
end
...
begin
	...
	res2 = Optimization.solve(optprob2, LBFGS(linesearch = BackTracking()), callback = callback, maxiters = 2000)
...
md"""
### DataDrivenProblem Package exploration  
**Purpose** = see if package was used correctly with NN approximation and simulated data.
Tested approaches:
- using the continuous DataDrivenProblem constructor (with an ODEsolution object and dX matrx)
- using the DirectDataDrivenProblem constructor

=> perfect equation discovery for both constructor using ground truth.
"""

# ╔═╡ 065bbb50-ad63-4416-8bf7-09939a66ef92
begin
	# Define a continuous data driven problem using ODEsolution
	#diff_dd_prob = DataDrivenProblem(X_ref_Xa)
	...
	# Define the basis with the differentials as implicits
	eqs_diff = [polynomial_basis(g, 3); D.(g); g[1] .* D(g[1]); g[1]^2 .* D(g[1]); g[2] .* D(g[2]); g[2]^2 .* D(g[2]); g[3] .* D(g[3]); g[3]^2 .* D(g[3]);]
...
begin
	...
	@variables j[1:3]
	j = collect(j)
	eqs_implicit = [polynomial_basis(g, 3); j; g[1] .* j[1]; g[1]^2 .* j[1]; g[2] .* j[2]; g[2]^2 .* j[2]; g[3] .* j[3]; g[3]^2 .* j[3]]

	# Define the basis with unknown function as implicits
	basis_direct = Basis(eqs_implicit, g, independent_variable = t, implicits = j)
end
...
git-tree-sha1 = "7df31056f34c5c8870d0fed624ef5b5a3adcf409"
uuid = "7da242da-08ed-463a-9acd-ee780be4f1d9"
version = "0.12.23"
weakdeps = ["ChainRulesCore", "LogExpFunctions", "SpecialFunctions", "StaticArrays"]
...
git-tree-sha1 = "cdbd3e370166f2f039a677c0c9cb58ecd373f258"
...
git-tree-sha1 = "bc87c2625861fc4a3e2e5920e89bc7c0bf4ba91f"

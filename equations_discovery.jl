...
# ╔═╡ 49891ff9-577b-4f0e-b792-df0c20b17a84
 using DataDrivenDiffEq, DataDrivenSparse, DataFrames, Plots, CSV, StableRNGs, ComponentArrays
...
begin
	path_to_data = "./Data/"
	egf_highCC_60m = CSV.read(Base.Filesystem.joinpath(path_to_data, "egf_highCC_60m.csv"), DataFrame)
	egf_highCC_10m = CSV.read(Base.Filesystem.joinpath(path_to_data, "egf_highCC_10m.csv"), DataFrame)
	ngf_highCC_60m = CSV.read(Base.Filesystem.joinpath(path_to_data, "ngf_highCC_60m.csv"), DataFrame)
	ngf_highCC_10m = CSV.read(Base.Filesystem.joinpath(path_to_data, "ngf_highCC_10m.csv"), DataFrame)
end
...
begin
	@variables v[1:3]
	nominator_basis = Basis(polynomial_basis(v, 2), v)
	#fn_basis = Basis([v[1], 1, v[2], v[1]v[2], 1/(1+(1-v[1])), 1/(1+(1-v[2]))], v)
	#fn_basis = Basis([nominator_basis; nominator_basis .+ 1], v)
	#fn_basis = Basis(reshape((nominator_basis ./ transpose(nominator_basis)), 100), v)
end

# ╔═╡ f5e76305-6af7-45f4-bb02-5f906a51f24a
[egf_highCC_60m.R_fit egf_highCC_60m.Ras_fit egf_highCC_60m.Raf_fit egf_highCC_60m.MEK_fit egf_highCC_60m.ERK_fit egf_highCC_60m.NFB_fit egf_highCC_60m.PFB_fit]

# ╔═╡ 8dbabaa2-4f3b-4468-9926-8761c44341e9
# ╠═╡ disabled = true
#=╠═╡
map(1:2) do i 
	DirectDataDrivenProblem(reshape(Array(egf_highCC_60m.R_fit), 1, 801), reshape(Array(egf_highCC_60m.NN_approx), 1, 801))
end

  ╠═╡ =#

# ╔═╡ dff10871-3962-484e-9e53-ef82d2725e8b
plot(egf_highCC_60m.time[101:end], egf_highCC_60m.NN_approx[101:end])

# ╔═╡ 09712a39-b970-4bfa-83f3-6ec4abf56144
begin
	w = (0 .* egf_highCC_60m.PFB_fit.* 
		(1 .- egf_highCC_60m.Ras_fit) ./ (0.01 .+ (1 .- egf_highCC_60m.Ras_fit)))
	plot!(egf_highCC_60m.time, w, label="Ground truth")
end

# ╔═╡ 491736b6-7434-4305-bceb-efe2a46a3e6c
begin
	X̂ = reshape([Array(egf_highCC_60m.Raf_fit[201:end]) Array(egf_highCC_60m.PFB_fit[201:end]) Array(egf_highCC_60m.NN_approx[201:end])], 3, 601)
	#X̂ = [egf_highCC_60m.R_fit egf_highCC_60m.Ras_fit egf_highCC_60m.Raf_fit egf_highCC_60m.MEK_fit egf_highCC_60m.ERK_fit egf_highCC_60m.NFB_fit egf_highCC_60m.PFB_fit]'
	Ŷ = reshape(Array(egf_highCC_60m.NN_approx[201:end]), 1, 601)
	#Ŷ = egf_highCC_60m.NN_approx'
	prob = DirectDataDrivenProblem(X̂, Ŷ)
end

# ╔═╡ 771afed6-5d16-4c10-9205-5d55af36752a
length(Array(egf_highCC_60m.PFB_fit[200:end]))

# ╔═╡ 9832e030-f895-4016-989a-b1e3bf0da6cf
X̂
...
begin
	...
	res = solve(prob, nominator_basis, opt, options = options)
...
manifest_format = "2.0"
project_hash = "e83dd06cb85b9cc6860d1f0bf8a0ac4f6c156523"
...
git-tree-sha1 = "500497753a6ab4a71ba8f7bcef6e09bd47074fc3"
...
git-tree-sha1 = "e31c460739fb1beb9d84cc851b670b43ef493fda"
...
git-tree-sha1 = "89c783f325e131233928bf108e17b75a8ec34857"
